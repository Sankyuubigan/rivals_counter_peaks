...
1. добавить в меню пункт который можно включить или выключить. по умолчанию выключен. он будет сохранять скриншоты экрана (у нас итак делается скриншот сейчас) если распозналось менее чем 6 героев. сохранять этот скриншот (без обрезания) в ту же директорию откуда запускалась программа (то есть рядом). давай при включении этого пункта из меню будем выводить окно выбора директории и можно будет указать директорию куда сохранять скриншоты эти.

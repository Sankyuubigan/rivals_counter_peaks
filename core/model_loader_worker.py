...
class ModelLoaderWorker(QObject):
    # Сигнал: success, ort_session, dino_embeddings, target_h, target_w
    models_loaded_signal = Signal(bool, object, dict, int, int)
    ...
    def run_load(self):
        ...
        if not self._ensure_dir_exists(nn_models_dir_abs) or \
           ...
           not self._ensure_dir_exists(onnx_model_dir_abs):
            ...
            self.models_loaded_signal.emit(False, None, {}, target_h_model_dino, target_w_model_dino)
            return
        ...
        try:
            # Оптимизация для быстрой загрузки модели - как в эталонном файле
            session_options = onnxruntime.SessionOptions()
            # Уменьшаем количество потоков для более быстрой инициализации
            session_options.intra_op_num_threads = 1
            session_options.inter_op_num_threads = 1
            # Отключаем оптимизацию графа для ускорения загрузки
            session_options.graph_optimization_level = onnxruntime.GraphOptimizationLevel.ORT_DISABLE_ALL
            # Отключаем параллельное выполнение
            ...
            logging.info(f"[ModelLoaderWorker] DINOv3 ONNX loaded. Target size: {target_w_model_dino}x{target_h_model_dino}")
        except Exception as e:
            logging.error(f"[ModelLoaderWorker] Error loading DINOv3 (ONNX session or processor): {e}", exc_info=True)
            self.models_loaded_signal.emit(False, None, {}, target_h_model_dino, target_w_model_dino)
            return
        embedding_files = [f for f in os.listdir(embeddings_dir_abs) if f.lower().endswith(".npy")]
        if not embedding_files:
            logging.error(f"[ModelLoaderWorker] No DINOv3 embeddings (.npy files) found in '{embeddings_dir_abs}'.")
        ...
        for emb_filename in embedding_files:
            ...
            try:
                ...
                logging.warning(f"[ModelLoaderWorker] Error loading DINOv3 embedding '{emb_filename}': {e}")
        ...
        if not dino_reference_embeddings and embedding_files: # Если были файлы, но ничего не загрузилось
            logging.error("[ModelLoaderWorker] Failed to load any DINOv3 embeddings from found files.")
            ...
            # return
        logging.info(f"[ModelLoaderWorker] Loaded DINOv3 embeddings: {len(dino_reference_embeddings)}")
        
        # Передаем input_name_dino неявно, он будет получен в AdvancedRecognitionLogic из сессии
        self.models_loaded_signal.emit(True, ort_session_dino, dino_reference_embeddings, target_h_model_dino, target_w_model_dino)

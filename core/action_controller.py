...
class ActionController:
    ...
    def handle_selection_changed(self):
        if getattr(self.mw, 'is_programmatically_updating_selection', False): return
    ...
    def handle_move_cursor(self, direction):
        ...
        if self.mw.hotkey_cursor_index < 0: new_index = 0
        else:
            ...
            if direction == 'left': new_index = self.mw.hotkey_cursor_index - 1 if current_col > 0 else (current_row * num_columns) - 1
            elif direction == 'right': new_index = self.mw.hotkey_cursor_index + 1 if current_col < num_columns - 1 else (current_row + 1) * num_columns
            elif direction == 'up': new_index = self.mw.hotkey_cursor_index - num_columns
            elif direction == 'down': new_index = self.mw.hotkey_cursor_index + num_columns
    ...
    def _move_window_in_tab_mode(self, direction):
        ...
        if not tray_window or not tray_window.isVisible(): return
        pos = tray_window.pos()
        x, y = pos.x(), pos.y()
        step = 50
    ...
    def handle_toggle_selection(self):
        is_tab_mode_active = getattr(self.mw, 'tab_mode_manager', None) and self.mw.tab_mode_manager.is_active()
        if is_tab_mode_active: return
        list_widget = getattr(self.mw, 'right_list_widget', None)
        if not list_widget or self.mw.hotkey_cursor_index < 0: return
    ...
    def handle_copy_team(self):
        utils_gui.copy_to_clipboard(self.mw.logic, show_message=False)

...
class RecognitionWorker(QObject):
    ...
    def __init__(self, hero_recognition_system: HeroRecognitionSystem,
                 recognition_area_dict: dict, start_time: float, parent=None):
        super().__init__(parent)
    ...
    def run(self):
        ...
        t2 = time.time()
        logging.info(f"[TIME-LOG] {t2 - self.start_time:.3f}s: "
                     f"Screenshot captured in {t2 - t1:.3f}s.")

        if screenshot is None:
            logging.error(f"[THREAD] Failed to capture screen area: "
                          f"{self.recognition_area_to_capture}")
            if self._is_running:
                self.error.emit(get_text('recognition_no_screenshot',
        ...
        try:
            ...
            if self._is_running:
                self.error.emit(f"Ошибка преобразования изображения: {e}")
            self.finished.emit([], screenshot, self.start_time)
            return

        t1_rec = time.time()
        recognized_heroes = self.hero_recognition_system.recognize_heroes_optimized(
            screenshot_pil)
        t2_rec = time.time()
        duration = t2_rec - t1_rec
        logging.info(f"[TIME-LOG] {t2_rec - self.start_time:.3f}s: "
                     f"Recognition inference took {duration:.3f}s.")
...
class RecognitionManager(QObject):
    ...
    def start_async_model_load(self):
        if self._models_ready or self._loader_thread is not None:
            logging.warning("[RecognitionManager] Model loading already started "
                            "or completed.")
            return
    ...
    def _on_models_loaded(self, success: bool):
        ...
        if success:
            logging.info("[RecognitionManager] Models and embeddings loaded "
                         "successfully.")
        else:
            logging.error("[RecognitionManager] Failed to load models or "
                          "embeddings.")
    ...
    def _on_recognition_complete(self, heroes: list, screenshot_np: np.ndarray | None,
                                 start_time: float):
        delta = time.time() - start_time
        logging.info(f"[TIME-LOG] {delta:.3f}s: RecognitionManager received "
                     f"results from worker.")
            ...
            if should_save:
                try:
                    ...
                    with mss() as sct:
                        ...
                        full_screen_canvas = Image.new("RGB", (screen_width, screen_height),
                                                       (128, 128, 128))
                        ...
                        # --- Правильные цвета: BGR→RGB ---
                        if screenshot_np.shape[2] == 3:
                            screenshot_rgb = np.flip(screenshot_np[:, :, :3], axis=2)
                        else:
                            screenshot_rgb = screenshot_np
                        roi_pil = Image.fromarray(screenshot_rgb).resize((width, height),
                                                                         Image.LANCZOS)
                        ...
                        text_pos = (10, 30)
                        draw.text((text_pos[0] + 1, text_pos[1] + 1),
                                  f"Recognized: {heroes_text}", fill="black", font=font)
                        draw.text(text_pos, f"Recognized: {heroes_text}",
    ...
    def _handle_recognize_heroes(self, start_time: float):
        ...
        if not self._models_ready:
            ...
            self.error.emit(get_text('recognition_models_not_ready',
                                     language=self.logic.DEFAULT_LANGUAGE))
            return

        self.is_recognizing = True
        self.recognition_started.emit()

        self._recognition_worker = RecognitionWorker(self.hero_recognition_system,
                                                     RECOGNITION_AREA, start_time)
    ...
    def _reset_recognition_state(self):
        ...
        if self._recognition_worker:
            self._recognition_worker.deleteLater()
        if self._recognition_thread:
            self._recognition_thread.deleteLater()

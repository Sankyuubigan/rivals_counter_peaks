...
class WindowFlagsManager:
    ...
    def apply_window_flags_and_show(self, new_flags: Qt.WindowFlags, reason: str):
        if self._is_applying_flags_operation:
            logging.debug(f"    [ApplyFlags] Skipped due to _is_applying_flags_operation already True. Reason: {reason}") # ИЗМЕНЕНО: DEBUG
            return
        ...
        try:
            ...
            logging.debug(f"    [ApplyFlags] Current actual flags: {current_actual_flags:#x}, New target flags: {new_flags:#x}, Need change: {flags_need_change}. Reason: {reason}")
            ...
            if flags_need_change:
                logging.debug(f"    [ApplyFlags] Applying new flags. Current geom: {self.mw.geometry()}") # ИЗМЕНЕНО: DEBUG
                self.mw.setWindowFlags(new_flags)
                self._last_applied_flags = new_flags 
                logging.debug(f"    [ApplyFlags] After setWindowFlags. New actual flags(): {self.mw.windowFlags():#x}. isVisible(): {self.mw.isVisible()}, geom: {self.mw.geometry()}") # ИЗМЕНЕНО: DEBUG

            if not self.mw.isMinimized():
                if not self.mw.isVisible():
                    logging.debug(f"    [ApplyFlags] Window is not visible (and not minimized). Calling show(). Reason: {reason}") # ИЗМЕНЕНО: DEBUG
                    t_show_start = time.perf_counter()
                    self.mw.show()
                    if self.mw.isVisible(): 
                        self.mw.setWindowIcon(self.mw.windowIcon()) 
                    logging.debug(f"        After show(): isVisible={self.mw.isVisible()}, isActiveWindow={self.mw.isActiveWindow()}. Time: {(time.perf_counter() - t_show_start)*1000:.2f} ms. Geom: {self.mw.geometry()}") # ИЗМЕНЕНО: DEBUG
                else: 
                    logging.debug(f"    [ApplyFlags] Window already visible or became visible after setWindowFlags. No explicit show() call needed now. Reason: {reason}. Geom: {self.mw.geometry()}") # ИЗМЕНЕНО: DEBUG
                    if self.mw.isVisible():
                        self.mw.setWindowIcon(self.mw.windowIcon())


                if self.mw.isVisible() and geom_before_operation.isValid() and self.mw.geometry() != geom_before_operation:
                    logging.debug(f"    [ApplyFlags] Restoring geometry from {self.mw.geometry()} to {geom_before_operation}. Reason: {reason}") # ИЗМЕНЕНО: DEBUG
                    self.mw.setGeometry(geom_before_operation)
            
            elif self.mw.isMinimized():
                 logging.debug(f"    [ApplyFlags] Window is minimized. No explicit show() or geometry restoration. Reason: {reason}") # ИЗМЕНЕНО: DEBUG
    ...
    def force_taskbar_update_internal(self, reason_suffix="unknown"):
        ...
        logging.debug(f"    [TaskbarUpdate] START force_taskbar_update_internal (Simplified). Caller reason: {caller_reason}") # ИЗМЕНЕНО: DEBUG
        ...
        logging.debug(f"    [TaskbarUpdate] END force_taskbar_update_internal (Simplified). Time: {(time.perf_counter() - t_start_force)*1000:.2f} ms") # ИЗМЕНЕНО: DEBUG
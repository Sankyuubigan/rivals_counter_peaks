...
# <<< ИЗМЕНЕНО: Возвращаем порог к 0.8, регулировка после анализа логов >>>
...
def validate_heroes():
    logging.info("[VALIDATION] Запуск проверки имен героев...") # Изменено с print на logging.info
...
def capture_screen_area(area: dict):
    logging.debug(f"[CAPTURE] Attempting capture for area definition: {area}") # DEBUG для деталей
    try:
        with mss.mss() as sct:
            ...
            if all(k in area for k in ['left_pct', 'top_pct', 'width_pct', 'height_pct']):
                use_pct = True; logging.debug(f"[CAPTURE] Using percentage values for area calculation.") # DEBUG
                ...
                logging.debug(f"[CAPTURE] Calculated px values: L={left_px}, T={top_px}, W={width_px}, H={height_px}") # DEBUG
            else:
                logging.debug(f"[CAPTURE] Using absolute pixel values from area definition.") # DEBUG
            ...
            logging.debug(f"[CAPTURE] Grabbing BBox: {bbox} on Monitor: {target_monitor_index}") # DEBUG
            ...
            logging.info(f"[CAPTURE] Area captured successfully. Shape: {img_bgr.shape}") # INFO об успехе
            return img_bgr

...
def clear_layout(layout):
    ...
    while layout.count():
        ...
        if widget is not None: widget.deleteLater()
            ...
            if sub_layout is not None: clear_layout(sub_layout)
...
def generate_minimal_icon_list(logic, result_frame, left_images):
    ...
    layout = result_frame.layout()
    if not layout: layout = QVBoxLayout(result_frame); layout.setObjectName("result_layout_min"); layout.setAlignment(Qt.AlignmentFlag.AlignTop); layout.setContentsMargins(2, 2, 2, 2); layout.setSpacing(1); result_frame.setLayout(layout)
    clear_layout(layout)
    ...
    for counter, score in filtered_counters:
        pixmap = left_images.get(counter)
        if is_invalid_pixmap(pixmap):
            logging.warning(f"[Minimal Display] Invalid/missing left image for hero '{counter}'. Skipping icon.")
            continue
        ...
        if is_selected: border_style = "border: 3px solid orange; border-radius: 3px; padding: 0px;"
        elif is_effective: border_style = "border: 3px solid blue; border-radius: 3px; padding: 0px;"
